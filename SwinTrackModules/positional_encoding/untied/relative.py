...
def generate_dualImg_2d_concatenated_self_attention_relative_positional_encoding_index(z_shape, x_shape):
    '''
        z_shape: (z_h, z_w)
        x_shape: (x_h, x_w)
    '''

    xz_2d_index_h, xz_2d_index_w = torch.meshgrid(torch.arange(z_shape[0]+x_shape[0]), torch.arange(z_shape[1]+x_shape[1]))  
    z_2d_index_h = xz_2d_index_h[:z_shape[0], :z_shape[1]]  
    z_2d_index_w = xz_2d_index_w[:z_shape[0], :z_shape[1]]  
    x_2d_index_h = xz_2d_index_h[z_shape[0]:, z_shape[1]:] 
    x_2d_index_w = xz_2d_index_w[z_shape[0]:, z_shape[1]:]  
...
def generate_2d_dualPos_concatenated_cross_attention_relative_positional_encoding_index(z_shape, x_shape):
    '''
        z_shape: (z_h, z_w)
        x_shape: (x_h, x_w)
    '''
